...
COMMIT; 
--**********************************************************************************************************************
--DESARROLLO INTRUCCIONES ESPECIFICAS
DECLARE
    -- Parámetros dinámicos
    v_mes_proceso NUMBER := 6; -- Mes de junio
    v_anno_proceso NUMBER := 2021; -- Año del proceso
    v_limite_asignacion CONSTANT NUMBER := 250000;

    -- Declarar VARRAY para los porcentajes
    TYPE porcentaje_array IS VARRAY(5) OF NUMBER(3, 2);
    v_porcentajes porcentaje_array := porcentaje_array(0.02, 0.04, 0.05, 0.07, 0.09);

    -- Cursores
    CURSOR cur_profesionales IS
        SELECT p.numrun_prof,
               p.nombre || ' ' || p.appaterno AS nombre_completo,
               pr.nombre_profesion AS profesion,
               COUNT(a.numrun_prof) AS nro_asesorias,
        ...
        WHERE EXTRACT(MONTH FROM a.inicio_asesoria) = v_mes_proceso
          AND EXTRACT(YEAR FROM a.inicio_asesoria) = v_anno_proceso
        GROUP BY p.numrun_prof, p.nombre, p.appaterno, pr.nombre_profesion,
                 p.cod_comuna, p.cod_tpcontrato, p.cod_profesion, c.nom_comuna
        ORDER BY pr.nombre_profesion, p.appaterno, p.nombre;

    CURSOR cur_tipo_contrato(p_cod_tpcontrato NUMBER) IS
        SELECT incentivo
        FROM tipo_contrato
        WHERE cod_tpcontrato = p_cod_tpcontrato;

    CURSOR cur_asig_profesion(p_cod_profesion NUMBER) IS
        SELECT asignacion
        FROM porcentaje_profesion
        WHERE cod_profesion = p_cod_profesion;

    -- Registros
    TYPE reg_profesional IS RECORD (
...
BEGIN
    ...
    -- Procesar datos para detalle_asignacion_mes
    FOR rec_profesionales IN cur_profesionales LOOP
        BEGIN
            -- Asignar valores al registro
            v_profesional.run_profesional := rec_profesionales.numrun_prof;
            v_profesional.nombre_profesional := rec_profesionales.nombre_completo;
            v_profesional.profesion := rec_profesionales.profesion;
            v_profesional.nro_asesorias := rec_profesionales.nro_asesorias;
            v_profesional.monto_honorarios := rec_profesionales.monto_honorarios;
            v_profesional.cod_comuna := rec_profesionales.cod_comuna;
            v_profesional.cod_tpcontrato := rec_profesionales.cod_tpcontrato;
            v_profesional.cod_profesion := rec_profesionales.cod_profesion;
            v_profesional.nom_comuna := UPPER(TRANSLATE(rec_profesionales.nom_comuna, 'ÁÉÍÓÚÑ', 'AEIOUN'));

            -- Calcular asignación por tipo de contrato
            BEGIN
                IF cur_tipo_contrato%ISOPEN THEN
                    CLOSE cur_tipo_contrato; 
                END IF;
                OPEN cur_tipo_contrato(v_profesional.cod_tpcontrato);
                FETCH cur_tipo_contrato INTO v_asig_tipo_contrato;
                IF cur_tipo_contrato%NOTFOUND THEN
                    RAISE NO_DATA_FOUND;
                ELSE
                    v_asig_tipo_contrato := ROUND((v_profesional.monto_honorarios * v_asig_tipo_contrato) / 100);
                END IF;
                CLOSE cur_tipo_contrato;
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    v_sql_error := 'ORA-01403: No se ha encontrado ningún dato';
                    v_asig_tipo_contrato := 0;
                    INSERT INTO errores_proceso (error_id, mensaje_error_oracle, mensaje_error_usr)
                    VALUES (sq_error.NEXTVAL, v_sql_error,
                            'Error al obtener porcentaje de asignación para el run Nro. ' || v_profesional.run_profesional);
            END;

            -- Calcular asignación de movilización extra
            CASE
                WHEN v_profesional.nom_comuna = 'SANTIAGO' THEN
                    v_monto_movil_extra := ROUND(v_profesional.monto_honorarios * v_porcentajes(1));
                WHEN v_profesional.nom_comuna = 'NUÑOA' THEN
                    v_monto_movil_extra := ROUND(v_profesional.monto_honorarios * v_porcentajes(2));
                WHEN v_profesional.nom_comuna = 'LA REINA' THEN
                    v_monto_movil_extra := ROUND(v_profesional.monto_honorarios * v_porcentajes(3));
                WHEN v_profesional.nom_comuna = 'LA FLORIDA' THEN
                    v_monto_movil_extra := ROUND(v_profesional.monto_honorarios * v_porcentajes(4));
                WHEN v_profesional.nom_comuna = 'MACUL' THEN
                    v_monto_movil_extra := ROUND(v_profesional.monto_honorarios * v_porcentajes(5));
                ELSE
            ...
            BEGIN
                IF cur_asig_profesion%ISOPEN THEN
                    CLOSE cur_asig_profesion; -- Cerrar si está abierto
                END IF;
                OPEN cur_asig_profesion(v_profesional.cod_profesion);
                FETCH cur_asig_profesion INTO v_asig_profesion;
                IF cur_asig_profesion%NOTFOUND THEN
                    RAISE NO_DATA_FOUND;
                ELSE
                    v_asig_profesion := ROUND((v_profesional.monto_honorarios * v_asig_profesion) / 100);
                END IF;
                CLOSE cur_asig_profesion;
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    v_sql_error := 'ORA-01403: No se ha encontrado ningún dato';
                    v_asig_profesion := 0;
            ...
            v_total_asignaciones := ROUND(v_monto_movil_extra + v_asig_tipo_contrato + v_asig_profesion);

            -- Validar límite de asignaciones
            IF v_total_asignaciones > v_limite_asignacion THEN
                INSERT INTO errores_proceso (error_id, mensaje_error_oracle, mensaje_error_usr)
                VALUES (sq_error.NEXTVAL, 'Error: profesional supera el monto límite de asignaciones. Run Nro. ' || v_profesional.run_profesional,
                        'Se reemplazó el monto total de las asignaciones calculadas de ' || v_total_asignaciones || ' por el monto límite de 250000');
            ...
            INSERT INTO detalle_asignacion_mes (
                ...
                nro_asesorias, monto_honorarios, monto_movil_extra,
                monto_asig_tipocont, monto_asig_profesion, monto_total_asignaciones
            ) VALUES (
                v_mes_proceso, v_anno_proceso, v_profesional.run_profesional, v_profesional.nombre_profesional, v_profesional.profesion,
                v_profesional.nro_asesorias, v_profesional.monto_honorarios, v_monto_movil_extra,
                v_asig_tipo_contrato, v_asig_profesion,
                v_total_asignaciones
            );
        END;
    END LOOP;

    -- Procesar datos para resumen_mes_profesion
    FOR rec_resumen IN (
        SELECT profesion,
               ROUND(SUM(nro_asesorias)) AS total_asesorias,
               ROUND(SUM(monto_honorarios)) AS monto_total_honorarios,
               ROUND(SUM(monto_movil_extra)) AS monto_total_movil_extra,
               ROUND(SUM(monto_asig_tipocont)) AS monto_total_asig_tipocont,
               ROUND(SUM(monto_asig_profesion)) AS monto_total_asig_prof,
               ROUND(SUM(monto_total_asignaciones)) AS monto_total_asignaciones
        FROM detalle_asignacion_mes
        GROUP BY profesion
        UNION ALL
        SELECT 'Abogado', 0, 0, 0, 0, 0, 0 FROM dual
        UNION ALL
        SELECT 'Arquitecto', 0, 0, 0, 0, 0, 0 FROM dual
        ORDER BY profesion
    ) LOOP
        INSERT INTO resumen_mes_profesion (
            ...
            v_anno_proceso * 100 + v_mes_proceso,
            rec_resumen.profesion,
            rec_resumen.total_asesorias,
            rec_resumen.monto_total_honorarios,
            rec_resumen.monto_total_movil_extra,
            rec_resumen.monto_total_asig_tipocont,
            rec_resumen.monto_total_asig_prof,
            rec_resumen.monto_total_asignaciones
        );
    END LOOP;
